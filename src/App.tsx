...
const App: React.FC = () => {
  ...
  const systemLoad = metrics.cpu === null ? 'N/A' : `${metrics.cpu.toFixed(0)}%`;
    ...
    <main className="nexus-shell w-full h-screen overflow-y-auto overflow-x-hidden bg-[#070B19] text-white">
      ...
      <div className="relative z-10 min-h-screen w-full flex flex-col gap-3 p-3">
        <header className="glass-panel shrink-0 flex min-h-14 items-center justify-between gap-4 px-4 py-3">
          <div className="flex min-w-0 items-center gap-4"><div className="flex shrink-0 items-center gap-2"><div className="w-3 h-3 border-2 border-[#00F0FF] rounded-sm rotate-45" /><h1 className="text-sm font-bold font-[Orbitron] tracking-wider text-[#00F0FF] neon-cyan whitespace-nowrap">NEXUS STRESS-HUD</h1></div><span className="hidden lg:block truncate border-l border-gray-700 pl-4 text-[9px] text-gray-500">LIVE DATA CONTROL SURFACE · ATIBON DEFENSIVE GATE</span></div>
