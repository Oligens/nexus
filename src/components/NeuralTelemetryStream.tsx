...
const NeuralTelemetryStream: React.FC = () => {
  const { entries, transportState, configured, connect, disconnect, clear, addOperatorNote } = useTelemetry();
  const [streaming, setStreaming] = useState(false);
  const [input, setInput] = useState('');
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => { if (containerRef.current) containerRef.current.scrollTop = containerRef.current.scrollHeight; }, [entries]);
  useEffect(() => { if (streaming) connect(); else disconnect(); return () => disconnect(); }, [streaming, connect, disconnect]);

  const levelClass = (level: 'info' | 'warning' | 'error') => ({ info: 'text-[#d6e2f0]', warning: 'text-[#D4AF37]', error: 'text-[#ff6b6b]' }[level]);
  const submitManualEntry = (event: React.FormEvent) => { event.preventDefault(); if (input.trim()) addOperatorNote(input); setInput(''); };
  ...
  return (
    <GlassPanel title="Neural Telemetry Stream" icon="⟁" className="h-full min-h-0 flex flex-col overflow-hidden">
      ...
      <form onSubmit={submitManualEntry} className="shrink-0 px-3 py-2 border-t border-[#D4AF37]/20 flex gap-2"><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Note opérateur locale..." className="nexus-input flex-1" aria-label="Operator note" /><button type="submit" disabled={!input.trim()} className="quick-mini px-3"><Terminal size={12} /> NOTE</button></form>
